...
from analysis.skill_alignment import analyze_skill_alignment
from analysis.ranking_module import rank_candidates
...
def process_single_cv(pdf_path: Path) -> dict:
    ...
    candidate_name = profile.get("personal_information", {}).get("name") or pdf_path.stem
...
def render_skill_tab(result):
    st.subheader("Skill Alignment Analysis")
    skills = result.get("skill_alignment", {})
    if not skills:
        st.info("No skill alignment data available.")
        return
        
    c1, c2 = st.columns(2)
    c1.markdown(f"<div style='margin-bottom: 15px;'><span style='font-size: 0.9rem; color: #a0a0a0;'>Overall Skill Score</span><br><span style='font-size: 1.6rem; font-weight: 600; line-height: 1.2;'>{skills.get('overall_skill_score', 0)}/100</span></div>", unsafe_allow_html=True)
    c2.markdown(f"<div style='margin-bottom: 15px;'><span style='font-size: 0.9rem; color: #a0a0a0;'>Alignment Label</span><br><span style='font-size: 1.6rem; font-weight: 600; line-height: 1.2;'>{skills.get('alignment_label', 'N/A')}</span></div>", unsafe_allow_html=True)
    
    table = skills.get("skill_table", [])
    if table:
        st.dataframe(pd.DataFrame(table), use_container_width=True, hide_index=True)
    st.info(skills.get("summary", ""))
...
def render_summary_tab(result):
    ...
    name = result["profile"].get("personal_information", {}).get("name") or result.get("filename", "Unknown")
...
def render_comparison_tab(results):
    ...
    comp_data = []
    for r in results:
        name = r["profile"].get("personal_information", {}).get("name") or r.get("filename", "Unknown")
        edu = r["education"]
        exp = r["experience"]
        res = r["research"]
        summ = r["summary"]
...
st.markdown(
    '<p class="sub-header">'
    'Talent Acquisition & Learning Automation for Smart Hiring - Milestone 3'
    '</p>',
    unsafe_allow_html=True
)
st.markdown("***")
...
if not results:
    ...
    if len(results) == 1:
        ...
        candidate_names = [
            r["profile"].get("personal_information", {}).get("name") or r["filename"]
            for r in results
        ]
